...
public class DialaState : GameState
{
    Unit player;

    public override void Enter()
    {
        Debug.Log("Entered Diala state");
        if (owner.gameType == 1)
        {
        ...
        Debug.Log("Diala has " + player.actions + " actions " + player.movement + " movement");
        ...
        {
            // disable all buttons but actions
            //owner.gameUI.transform.GetChild(0).GetChild(1).GetComponent<Button>().interactable = false;
            //owner.gameUI.transform.GetChild(0).GetChild(2).GetComponent<Button>().interactable = false;
            //owner.gameUI.transform.GetChild(0).GetChild(3).GetComponent<Button>().interactable = false;
            player.actions--;
            player.stun = false;
    ...
    void checkConditions()
    {
        ...
        if (player.stun)
        {
            owner.gameUI.transform.GetChild(0).GetChild(7 + amount).GetComponent<Image>().sprite = owner.data.conditions[0];
            owner.gameUI.transform.GetChild(0).GetChild(7 + amount).GetComponent<Image>().color = Color.white;
            amount++;
        }

        if (player.bleed)
        {
            owner.gameUI.transform.GetChild(0).GetChild(7 + amount).GetComponent<Image>().sprite = owner.data.conditions[1];
            owner.gameUI.transform.GetChild(0).GetChild(7 + amount).GetComponent<Image>().color = Color.white;
            amount++;
        }
        ...
        if (player.weaken)
        {
            owner.gameUI.transform.GetChild(0).GetChild(7 + amount).GetComponent<Image>().sprite = owner.data.conditions[2];
            owner.gameUI.transform.GetChild(0).GetChild(7 + amount).GetComponent<Image>().color = Color.white;
            amount++;
        }
        ...
        if (player.focused)
        {
            owner.gameUI.transform.GetChild(0).GetChild(7 + amount).GetComponent<Image>().sprite = owner.data.conditions[3];
            owner.gameUI.transform.GetChild(0).GetChild(7 + amount).GetComponent<Image>().color = Color.white;
            amount++;
        }
        ...
        if (player.hidden)
        {
            owner.gameUI.transform.GetChild(0).GetChild(7 + amount).GetComponent<Image>().sprite = owner.data.conditions[4];
            owner.gameUI.transform.GetChild(0).GetChild(7 + amount).GetComponent<Image>().color = Color.white;
        }

        for (int i = amount; i < 3; i++)
        {
            owner.gameUI.transform.GetChild(0).GetChild(7 + amount).GetComponent<Image>().color = Color.clear;
        }
    }
